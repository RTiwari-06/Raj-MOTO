...
export default function ProjectDetail({ ride, rides, rideIndex, onClose, onNavigate }) {
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (e.key === 'ArrowLeft'  && hasPrev) onNavigate(rideIndex - 1);
      if (e.key === 'ArrowRight' && hasNext) onNavigate(rideIndex + 1);
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
  }, [handleClose, hasPrev, hasNext, rideIndex, onNavigate]);
      ...
      >
        <div className="max-w-screen-xl mx-auto flex items-center justify-between">
          ...
          {hasPrev ? (
            <button
              data-magnetic
              onClick={() => onNavigate(rideIndex - 1)}
